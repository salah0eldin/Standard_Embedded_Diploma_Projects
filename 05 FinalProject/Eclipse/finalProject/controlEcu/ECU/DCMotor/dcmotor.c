 ...
 *                              Global Variables                               *
 *******************************************************************************/
DCMOTOR_t g_dcmotor = { .dirPort = PORTD_ID, .dirPin1 = PIN6_ID, .dirPin2 =
		PIN7_ID, .enableEn = 1, .enPort = PORTB_ID, .enPin = PIN3_ID };
...
void DCMOTOR_init(DCMOTOR_t *dcmotor) {
	...
	if (dcmotor->enableEn) {
		...
		GPIO_writePin(dcmotor->enPort, dcmotor->enPin, LOGIC_HIGH);
	}
